...
export const createProject = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const prompt = {
            ...
            Match lighting, shadows, and scale and perspective. 
...
export const createVideo = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if(!project || project.isGenerating) {
            return res.status(404).json({ message: "Generation in process" });
        }

        if(project.generatedVideo) {
            return res.status(201).json({message: "Video already generated"});
