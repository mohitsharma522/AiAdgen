...
function ProjectCard({ gen, setGenerations, forCommunity = false }: { gen: Projects, setGenerations: React.Dispatch<React.SetStateAction<Projects[]>>, forCommunity?: boolean }) {
    ...
    const togglePublish = async (projectId: string)=> {
        try {
            ...
            setGenerations((generation)=> generation.map((gen)=> gen.id === projectId ? {...gen, isPublished: data.isPublished} :gen));
                    ...
                    {!forCommunity && (
                        <div
                            ...
                            <div className="flex flex-col items-end w-32 text-sm">
                                <ul className={`text-xs ${MenuOpen ? 'block' : 'hidden'} overflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 backdrop-blur-2xl text-white border border-gray-500/50 rounder-lg shadow-md mt-2 py-1 z-10 `}>
                                    ...
                                    {(gen.generatedVideo || gen.generatedImage) && <button
                                        onClick={() => navigator.share({ url: gen.generatedVideo || gen.generatedImage, title: gen.productName, text: gen.productDescription })} className="w-full flex gap-2 items-center px-4 py-2 hover: bg-black/10 cursor-pointer " >
                                        <Share2Icon size={14} /> Share
                                    </button>}

                                    <button onClick={()=> handleDelete(gen.id)} className="w-full flex items-center gap-2 px-4 py-2 hover:bg-red-950/10 text-red-400 cursor-pointer ">
